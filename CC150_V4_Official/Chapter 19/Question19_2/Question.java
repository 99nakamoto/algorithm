...
public class Question {
	...
	public static Piece hasWon(Piece[][] board) {
		...
		for (int i = 0; i < N; i++) { // for each row and column
			...
			for (int j = 0; j < N; j++) {
				if (board[i][j] == board[i][0] && board[i][0] != Piece.Empty) {
				...
				/*
				 * Allow breaking early if you know no one can win this row or
				 * col.
				 */
	...
	public static void main(String[] args) {
		...
		pc = hasWon(board);
		System.out.println("hasWon1(): " + pc);
		System.out.println();

		pc = hasWon2(board);
		System.out.println("hasWon2(): " + pc);
		System.out.println();

		pc = MyAnswer.hasWon3(board);
